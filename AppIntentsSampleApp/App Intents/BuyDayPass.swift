...
struct BuyDayPass: AppIntent {
    static var title: LocalizedStringResource = "Buy Day Pass"
    static var description = IntentDescription("Purchase a day pass for your trail activity with Apple Pay.")

    @MainActor
    func perform() async throws -> some IntentResult & ProvidesDialog {
        /**
         Some types of intents should confirm with people that they want to complete the action, such as intents involving a financial transaction.
        ...
        do {
            ...
            try await requestConfirmation(result: .result(dialog: confirmationDialog), confirmationActionName: .buy)
            
            Logger.intentLogging.debug("[BuyDayPass] User confirmed they want to make the transaction")
        } catch {
            Logger.intentLogging.debug("[BuyDayPass] User canceled transaction at confirmation step")
             ...
             */
            throw TrailIntentError.dayPassTransactionCancelled
        }
