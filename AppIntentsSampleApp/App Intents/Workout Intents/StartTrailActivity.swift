...
struct StartTrailActivity: StartWorkoutIntent {
    ...
    static var title: LocalizedStringResource = "Start Trail Activity"
     ...
     */
    static var description = IntentDescription("Starts tracking a trail activity.",
    ...
    /**
     ...
     or prompt people for HealthKit authorization.
     */
    static var openAppWhenRun: Bool = true

    /**
     These suggestions are offered as options in the Settings app, when people configure which workout to start by pressing the Action button,
     and in the Shortcuts app.
     */
    static var suggestedWorkouts: [StartTrailActivity] = [
    ...
    func perform() async throws -> some IntentResult {
        ...
        if activityTracker.isActivityTrackingAuthorized {
            try await startActivityTracking()
        } else {
            /**
             When a person presses the Action button on Apple Watch Ultra, the watch will display an orange overlay until the intent returns a
             result. If the person hasn't already granted HealthKit authorization for the app, this overlay will cover the HealthKit
             authorization screen. Instead, request HealthKit authorization via a `Task`, allowing this intent to return quickly.
             */
    ...
    private func startActivityTracking() async throws {
        ...
        // Once the activity tracking starts, display the trail in the UI so that people can see the tracking in the Activity History.
