...
Abstract:
An object that keeps track of the specific activities someone did on a trail, such as a hike.
*/
...
@Observable class ActivityTracker: ActivityTrackingActions, @unchecked Sendable {
    ...
    /// This is nil except on watchOS, where it records a workout into `HKHealthStore`.
    private var liveWorkoutManager: ActivityTrackingActions?

    /// This property is always `true` unless it's running on watchOS, where it represents HealthKit authorization status.
    var isActivityTrackingAuthorized: Bool
    
    /// On watchOS, this presents a HealthKit authorization. On other platforms, it directly returns `isActivityTrackingAuthorized`.
    ...
    /// Ends the new activity, updating the activity with the time it completed.
    @MainActor
    func endActivity() {
    ...
    var activityStatistics: ActivityStatisticsSummary {
        ...
        for activity in activityHistory {
            ...
            stats.totalCaloriesBurned += 100
        }
        stats.totalWorkoutsCompleted = activityHistory.count
        return stats
    }
...
private class LiveWorkoutManager: NSObject, ActivityTrackingActions {
    ...
    /// Data store containing a person's health data.
    ...
    func requestActivityTrackingAuthorization() async throws -> Bool {
        ...
        do {
            ...
            isActivityTrackingAuthorized = false
            Logger.activityTracking.error("Error requesting HealthKit auth \(String(describing: error))")
            throw error
        }
    ...
    func startNewActivity(_ activity: ActivityStyle, on trail: Trail) async throws {
        ...
        do {
            ...
            Logger.activityTracking.error("Could not create the workout session. \(String(describing: error))")
            throw error
        }

        // Sets builder's data source to the instance's `HKHealthStore` object.
        ...
        do {
            ...
            Logger.activityTracking.error("Workout builder could not begin data collection. \(String(describing: error))")
            throw error
        }
    }
...
/**
 Defines an interface for both iOS and watchOS activity tracking, to separate the simple activity tracking on iOS from the
 live workout tracking APIs only available on watchOS with minimal `#if os(watchOS)` lines breaking up the code.
 */
